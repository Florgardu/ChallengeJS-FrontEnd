      ...
      <div v-if="this.$store.state.datosForm.length" class="table-responsive">
          <table class="table">
            ...
            >
              ...
              <td>{{ dato.tipo }}</td>
              <td>
              <button type="button" class="btn btn-warning" @click="editarOperacion(dato.nro_id)">Editar</button>
              </td>
               <td>
              <button type="button" class="btn btn-danger" @click="eliminarOperacion(dato)">Eliminar</button>
              </td> 
            </tr>
  ...
  export default  {
    ...
    methods: {
      ...
      eliminarOperacion(dato){
        ...
        if (confirm(`Esta seguro que desea eliminar esta operacion: "${dato.concepto}" ?`)) {
        try {
